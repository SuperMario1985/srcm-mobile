			...
			<view class="picker-modal-footer">
				<view class="picker-modal-footer-info">
					<block v-if="isMultiSelect">
						<view class="picker-display" style="margin-bottom: 8px;">
							...
							<view v-if="isContainTime" class="picker-display-link" :hover-stay-time="100"
							      ...
							      :style="{color}" @click="onShowTimePicker('begin')">{{BeginTimeTitle}}
							</view>
						</view>
						<view class="picker-display">
							...
							<view v-if="isContainTime" class="picker-display-link" :hover-stay-time="100"
							      hover-class="picker-display-link-active"
							      :style="{color}" @click="onShowTimePicker('end')">{{EndTimeTitle}}
							</view>
						</view>
					</block>
					<block v-else>
				...
				<view class="picker-modal-footer">
					...
					<view class="picker-modal-footer-btn">
						<view class="picker-btn" :hover-stay-time="100" hover-class="picker-btn-active"
						      @click="onCancelTime">取消
						</view>
						<view class="picker-btn" :style="{color}" :hover-stay-time="100" hover-class="picker-btn-active"
						      @click="onConfirmTime">确定
						</view>
					</view>
				</view>
			</view>
			...
			onConfirm () {
				...
				if (this.type == 'time') {
					...
					if (this.isMultiSelect) {
						...
						if (route == 'pages/group/detail' && this.checkeds.length < 1) {
						...
						let z = date1.getDate()
						z = z < 10 ? ('0' + z) : z;
						let time1 = date1.getFullYear() + "-" + m + "-" + z;
						let time2 = new Date(time1)
						let time3 = time2.getTime(time2)


						let d = new Date(result.value[0])
						let t = d.getTime(d)
						let n = new Date(result.value[1])
						let T = n.getTime(n)
						let s = (T - t) / (24 * 60 * 60 * 1000)
						let h = (T - time3) / (24 * 60 * 60 * 1000)
						let i = (t - time3) / (24 * 60 * 60 * 1000)
						// if(h>0 || i>0 ) {
						...
						// 	})
						// }
						if (route == 'pages/scrm/member_data' || route == 'pages/scrm/groupData') {
							if (h == 0 || i == 0) {
								return uni.showToast({
									icon : 'none',
									title: '不能选择今天'
						...
						if (route == 'pages/scrm/contents') {
							if (page[0].tabActiveIdx == 0) {
								if (s > 29) {
									return uni.showToast({
										icon : 'none',
										title: '所选范围不能超过三十天'
						...
						if (route == 'pages/scrm/statistics' || route == 'pages/scrm/DetailedData' || route == 'pages/scrm/follow_up_data' || route == 'pages/scrm/groupData' || route == 'pages/scrm/member_data') {
							if (s > 29) {
								return uni.showToast({
									icon : 'none',
									title: '所选范围不能超过三十天'
...
<style lang="scss" scoped>
	...
	.picker {
		...
		&-time {
			width: $calendar-size - 40px !important;
			left: ((375px - $calendar-size) / 2 + 20px) !important;
		}
