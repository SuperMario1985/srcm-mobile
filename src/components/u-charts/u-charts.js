...
var config = {
	...
	colors              : ['#1890FF', '#DB7093', '#DDA0DD', '#4B0082', '#6A5ACD', '#00FFFF', '#48D1CC', '#FFFF00', '#808080', '#FF7F50', '#191970', '#FFF0F5'],
...
function drawPieText (series, opts, config, context, radius, center) {
	...
	var seriesConvert = series.map(function (item) {
		var text = item.format ? item.format(+item._proportion_.toFixed(3)) : util.toFixed(item._proportion_ * 100) + '%';
...
function drawFunnelText (series, opts, context, eachSpacing, labelAlign, activeWidth, centerPosition) {
	...
	for (let i = 0; i < series.length; i++) {
		...
		let text = item.format ? item.format(+item.data.toFixed(2)) : util.toFixed(item.data) + '%';
		if(item.data==0){
			sum++
		}
		if (labelAlign == 'right') {
	...
	if(sum==series.length){
		for(let a=0;a < series.length; a++){
			series[a].area=[ 141.47500000000002,223,232.52500000000003,241]
			series[a].funnelArea=[141.75686215689217, 35.29999999999998,233.24313784310783,55.599999999999994]
		}
	}
}
