<template>
	<scroll-view v-if="ready" @scrolltolower="scroll" scroll-y="true" class="index">
		...
		<view class="detail" v-if="send_money != 0 || is_expired == 1">
			<text v-if="is_expired == 1">红包已过期，</text>
			<text v-if="send_money != 0">已领取{{get_num}}个，共{{get_amount}}/{{redpacket_amount}}元</text></view>
		<view class="detail" v-if="send_money == 0 && sendList.length == 0 && is_expired != 1">红包待领取</view>
		...
		<view style="color: #B0B0B0;padding: 0.8rem /* 15/18.75 */ 0;font-size: 0.693rem /* 13/18.75 */;"
		      v-if="count == sendList.length && sendList.length > 0">已加载全部
		</view>
...
<script>
	...
	export default {
		...
		mounted () {
			...
			// /groupRedpacketSend/list?corp_id=ww93caebeee67d134b&assist=groupRedpacket_885_191_0&external_userid=wmiWVTDwAAxUUGEGHXbblk--X5v40m0w
			// this.getData()
		...
		methods: {
			...
			async getData (page = this.page) {
				...
				const {data: res} = await this.axios.post("redpacket-open/group-send-redpacket-receive", {
					external_userid: localStorage.getItem('user_id') ? '' : (this.getParameter('external_userid') != null ? this.getParameter('external_userid') : localStorage.getItem('external_id')),
...
<style scoped>
	...
	.get-desc {
		...
		margin-bottom: 1.92rem /* 36/18.75 */;
