					...
					<view class="backGroundImage">
						<view style="margin-top: 2.7rem;">
							...
							<view style="font-size: 0.7rem;margin-top: 0.1rem !important;">
								<view class="text-p" v-for="(item, index) in levels"
								      style="padding: 0 2.667rem  /* 50/18.75 */;">
									<text>奖品：</text>
									<text v-if="item.type == 1">{{item.prize_name}}，剩余{{item.num}}个</text>
									<text v-if="item.type == 2">红包{{item.money_amount}}元，剩余{{item.money_count}}个</text>
								</view>
							</view>
...
<script>
	...
	export default {
		...
		inject : ['getParameter','getCopyright'],
		methods: {
			async getData () {
				let that = this
				Toast.loading({
				...
				const {data: res} = await this.axios.post("chat-message/activity-preview", {
				...
				if (res.error != 0) {
					...
					if (res.data.run == 1) {
						window.location.href = this.$store.state.h5Url + '/h5/pages/marketFission/index' + window.location.search
					} else {
						...
						this.back_pic_url = res.data.poster.background_url
						let img = new Image()
