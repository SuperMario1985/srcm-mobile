<template>
	<view class="member_data_container ">
			...
			<view class="header">
				<view>
					<view>
						...
						<text class="data">成员联系客户群统计</text>
					</view>
				</view>
			...
			<view>
				<view style="display: flex;align-items: center;height: 2.533rem;color: #999;background: #FFF;z-index: 150">
					<view v-show="show==0" @click="chooseStaff"
					      style="padding: 0.2rem 0;flex-grow: 1;;text-align: center;height: 2.133rem;line-height: 2.133rem;font-weight: bold;font-size: 0.747rem">
					...
					<ms-dropdown-menu style="
						...
						color: #333333; ">
						<ms-dropdown-item
								style="height: 2.133rem;font-size: 0.747rem;display:flex;align-items: center; justify-content: center; font-weight: bold;color: #999"
								:hasSlot="true" :title="timeName" ref="dropdownItemD">
							<view class="dropdown-item-content"
							      style="z-index: 999999999999999999999999;line-height: 2.133rem;text-align: center">
		...
		<!--  按员工列表	-->
		<scroll-view style="top: 4.932rem; left: 0; right: 0; bottom: 0; position: fixed;"
			...
			<view style="position: relative;">
				<view class="left" style="z-index: 50; position: absolute;top: 0;left: 0;">
					...
					<view style="position: relative;top: 1.6rem; margin-bottom: 1.6rem;">
						<view class="item" v-for="(item,index) in usersInfo" :key="index">
							<view class="title">
				...
				<view class="right" style="z-index: 30;">
					<scroll-view scroll-left="6.88rem" ref="scrollBox"
					             class="scroll-view_H " scroll-x="true"
					             @scroll="scroll"
					>
						<view style="width:100%" class="scroll-view-item_H uni-bg-green">
							<view class="scroll_left"
							      style="width:48.16rem;z-index: 30; position: fixed;
							      display: flex;left: 6.88rem;background: rgb(243, 243, 247)">
								<view style="align-items: center;display: flex;">
									<view class="thB">新增群聊数</view>
									<view class="thB">群总数</view>
									<view class="thB">有过消息的群聊数</view>
									<view class="thB">新增群成员数</view>
									<view class="thB">群成员总数</view>
									<view class="thB">发过消息的群成员数</view>
									<view class="thB">群聊消息总数</view>
								</view>
							</view>
							<view
									style="width:48.16rem; z-index: 0;position: relative;top: 1.6rem; margin-bottom: 1.6rem; background: #FFF">
								<view v-for="(item,index) in usersInfo" :key="index"
								      style="align-items: center;display: flex;">
									<view class="thcon">{{item.new_chat_cnt_snum}}</view>
									<view class="thcon">{{item.chat_total_snum}}</view>
									<view class="thcon">{{item.chat_has_msg_snum}}</view>
									<view class="thcon">{{item.new_member_cnt_snum}}</view>
									<view class="thcon">{{item.member_total_snum}}</view>
									<view class="thcon">{{item.member_has_msg_snum}}</view>
									<view class="thcon">{{item.msg_total_snum}}</view>
								</view>
							</view>
						</view>
					</scroll-view>
			...
			<view class="custom-tabs-loading" v-if="usersInfo.length > 0">{{loadingText}}</view>
...
<script>
	...
	export default {
		...
		onReachBottom: function () {
			if (this.usersInfo.length < this.count) {
				this.page += 1
				this.getMemberGroupData(this.page)
			} else {
		...
		methods      : {
			...
			goBack () {
				...
				let userIdx = localStorage.getItem('tab2') !== null ? localStorage.getItem('tab2') : 1
				...
				window.location.href = this.$store.state.h5Url + '/h5/pages/scrm/contents?tax=0&uix=' + userIdx + '&agent_id=' + agent_id
				// uni.navigateBack()
				uni.reLaunch({
					url: '/pages/scrm/contents'
				});
			},
			async getMemberGroupData (page = 1) {
				uni.showLoading({
					title   : '加载中...',
					duration: 2000
				});
				...
				const {data: res} = await this.axios.post('wap-user-desktop/work-user-chat-statistic-list', {
				...
				if (res.error != 0) {
					...
					if (this.page == 1) {
						...
						this.usersInfo = this.usersInfo.concat(res.data.list)
					}
					this.isLoading = false
					if (this.usersInfo.length == this.count) {
						this.isLoading = true
